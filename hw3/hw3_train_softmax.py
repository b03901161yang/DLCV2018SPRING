...
def FCN_32(input_shape=None, num_classes=2):
    ...
    o = Conv2DTranspose(filters = num_classes, kernel_size = (64, 64), strides = (32, 32), padding='same', activation ='softmax', name='up32')(o)

    model_fcn = Model(img_input, o)

    return model_fcn

#############################################
checkpointer = ModelCheckpoint(filepath='fcn32_softmax.h5', verbose=1)
model = FCN_32(input_shape = input_shape, num_classes = num_classes)
model.summary()

finetune = 0
if(finetune):
    model.load_weights('fcn32_softmax.h5')
    lr = 0.00001
...
model.save("fcn32_softmax.h5") #fcn32_softmax_iou682.h5
print("model saved")
